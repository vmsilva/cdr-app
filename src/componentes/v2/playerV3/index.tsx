...
import Video, { ResizeMode, VideoRef, TextTrackType, IgnoreSilentSwitchType } from "react-native-video";
...
const PlayerV3: React.FC<VideoPropsComponent | any> = ({
  ...
  const hideTimeout = useRef(null); // Timeout para ocultar os controles
  ...
  const controlsOpacity = useRef(new Animated.Value(1)).current;
   ...
   // Função para mostrar os controles e iniciar o timeout para ocultá-los
   const handleShowControlls = () => {
    ...
    resetHideTimeout(); // Reseta o tempo para ocultar os controles
  };

  // Reseta o tempo para ocultar os controles
        ...
        {reload ? (
          ...
          <>
            <TouchableOpacity onPress={!pause ? handlePause : handlePlay}>
              ...
              {
                !pause ? <SVGPAUSE width={SIZEICON} height={SIZEICON}/> : <SVGPLAY width={SIZEICON} height={SIZEICON}/>
              }
  ...
  }, [pause, finalizado, width, status, reload, pip, subtituloSelecionado, legendas_video]);
  ...
  const controll16x9 = useMemo(() => {
    ...
    return (
      <Controls_16_9
        ...
        legendas_video={legendas_video ? legendas_video : []}
        subtituloSelecionado={subtituloSelecionado}
        aovivo={aovivo}
        backButton={backButton}
        status={status}
        ...
        >
...

          {
            <Video
              ref={video}
              source={{
                uri: item.hls//HLSERROR ? item.mp4 : item.hls, //'http://www.fookaijafoa.com/'//item.hls,
              ...
              minLoadRetryCount={3}
              //controls
              ignoreSilentSwitch={IgnoreSilentSwitchType.INHERIT}
              ...
              onLoad={(event) => {
                setInfos(event);
                console.log(event, 'evennnnnnt ->> victorio')
                setIsLoad(false);
                if (seek_video != "" && seek_video != undefined){
                  //alert(seek_video)
                  seek(parseInt(seek_video));
                  console.log(seek_video, "carrega player onloadddd", item.seek_video, item.segundos_assistidos);
                }
              }}
