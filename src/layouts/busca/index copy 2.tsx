...
import ScrollHorizontal from "../../componentes/body/scrollHorizontal";
...
import { FAKECATEGORIA } from "../../configuracoes/utils/constants/utils";
...
const [debouncedTermoBusca, setDebouncedTermoBusca] = useState("");
  ...
  const handleOpenUrl = (item: any) => {
    navigation.navigate("SinopseLivroDrawer", {
      item: item,
      pagina_origem: "BuscarNavigatorStack",
    });
  };
  ...
  }, [isFocused, retornoBusca]);

  useEffect(() => {
    const timeoutId = setTimeout(() => {
      setDebouncedTermoBusca(termoBusca);
    }, 1000); // 500ms de atraso após o usuário parar de digitar
    
    return () => {
      clearTimeout(timeoutId); // Limpa o timeout quando `termoBusca` muda
    };
  }, [termoBusca]);

  useEffect(() => {
    if (debouncedTermoBusca) {
      //console.log('buscaaaaatermos______')
      buscaTermo(); // Executa a busca somente após o debounce
    }
  }, [debouncedTermoBusca]);
